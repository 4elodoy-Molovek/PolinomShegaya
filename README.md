...
* Таблицы должны быть 6 видов: линейная на массиве, линейная на списке, упорядоченная на массиве, дерево (АВЛ или красно-чёрное), две хэш-таблицы.
...
* Операции над таблицами: добавление полинома (во все сразу), удаление полинома (из всех сразу), поиск (только в активной таблице, выполняется в процессе вычисления выражений, составленных из имен полиномов).
...
* Требования к классу *Polynomial* (Полином)
  ...
  *Polynomial* - класс, содержащий информацию о полиноме и его программном представлении.
  ...
  - Степень любой переменной в любом мономе всегда меньше 10 и не меньше 0;
  ...
  - Класс должен иметь функционал сложения двух полиномов, вычитания одного полинома из другого, вычисления значения полинома в точке трёхмерного пространства, взятия производной по указанной переменной, интегрирования по указанной переменной, перемножения двух полиномов, умножения полинома на константу. Все арифметические операции должны выполняться через постфиксную форму;
  
  - Если в результате операции получается полином, то **по желанию пользователя** он сохраняется в таблицы до конца выполнения программы.

## Проект пользовательского интерфейса

![Проект пользовательского интерфейса](App_Design.png)

Пользовательский интерфейс состоит из главного и всплывающего окон. В главном окне справа расположена таблица полиномов, состоящая из колонок `имя полинома` и `строковая нотация полинома`. Выше таблицы находится селектор для смены активной таблицы.
Под таблицей находятся кнопки:
- `rename` позволяющая выбрать и переименовать полином;
- `delete` позволяющая выбрать и удалить полином;
- `add`, позволяющая добавить полином в таблицу через всплывающее окно.

В левой части главного окна находятся:
- поле ввода арифметического выражения, использующего имена полиномов в качестве переменных, с кнопкой для вычисления результата;
- поле результата, где будет выведен результат вычисления арифметического выражения введённого пользователем;
- кнопка сохранения результата, вызывающая всплывающее окно с введённым в нём полиномом-результатом.

Всплывающее окно состоит из поля для ввода имени полинома, поля ввода полинома и кнопки подтверждающей сохранение введённых данных.

## Общая структура программы

![Структура](Program_Structure_new.png)
...
- *PolynomialHandler* - класс, отвечающий за интерпретацию, хранение полиномов и произведение операций над ними, агрегирует следующие компоненты:
  ...
  - Объект класса *PolynomialParser*, отвечающий за перевод строковой нотации полинома в его программное представление в виде объекта200 класса Polynomial;
...
- Умножение полиномов;
	Попарное перемножение всех мономов в соответствии с правилами алгебры.
...
- Разбивает строковое представление полинома на лексемы;
    Происходит проходом по всей строке с учётом лексических правил написания полиномов; ==!!!==
...
## Expression Analyzer
Объект, анализирующий выражения содержащие полиномы, для их дальнейшего вычисления.
